...
document.addEventListener('DOMContentLoaded', () => {
  ...
  themeToggle.addEventListener('click', () => {
    document.body.className = document.body.classList.contains('light-mode') ? 'dark-mode' : 'light-mode';
  ...
  for (let i = 0; i < totalLeaves; i++) {
    ...
    frontFace.innerHTML = `<div class="page-header"><span>K-point E74 (২০২৬)</span><span>পৃষ্ঠা ${frontPageData.num}</span></div><div class="page-content" data-page="${frontPageData.num}">${frontPageData.content}</div>`;
    leaf.appendChild(frontFace);

    const backFace = document.createElement('div');
    backFace.className = 'page-face back';
    backFace.innerHTML = backPageData ? `<div class="page-header"><span>K-point E74 (২০২৬)</span><span>পৃষ্ঠা ${backPageData.num}</span></div><div class="page-content" data-page="${backPageData.num}">${backPageData.content}</div>` : `<div class="page-header"><span>K-point E74 (২০২৬)</span><span>সমাপ্ত</span></div><div class="page-content" style="display:flex;align-items:center;justify-content:center;flex-direction:column;text-align:center;"><i class="fa-solid fa-file-signature" style="font-size:48px;color:var(--color-primary);margin-bottom:16px;"></i><h2 style="border:none;background:none;">সমাপ্ত</h2></div>`;
  ...
  let touchStartX = 0;
  book3d.addEventListener('touchstart', e => { touchStartX = e.changedTouches[0].screenX; }, { passive: true });
  book3d.addEventListener('touchend', e => {
    const xDiff = touchStartX - e.changedTouches[0].screenX;
    if (Math.abs(xDiff) > 40) xDiff > 0 ? nextPage() : prevPage();
  }, { passive: true });

  searchInput.addEventListener('input', () => {
    const query = searchInput.value.trim().toLowerCase();
    if (!query) { clearSearch.style.display = 'none'; searchResults.style.display = 'none'; return; }
    ...
    searchResults.innerHTML = matches.length ? matches.map(m => `<div class="search-item" data-page="${m.pageNum}"><div class="match-title">পৃষ্ঠা ${m.pageNum} - ${m.title}</div><div class="match-snippet">${m.snippet}</div></div>`).join('') : '<div class="search-item" style="text-align:center;color:var(--text-muted);">কোনো ফলাফল পাওয়া যায়নি</div>';
  ...
  clearSearch.addEventListener('click', () => { searchInput.value = ''; clearSearch.style.display = 'none'; searchResults.style.display = 'none'; });
